...
void	*philo1(void *arg)
{
	...
	while (1)
	{
		...
		print_safe(p, time_stamp(p->base), 1, "has taken a fork");
		start_eat = time_stamp(p->base);
		print_safe(p, start_eat, 1, "is eating");
		if (read_stop(p))
			break ;
		...
		print_safe(p, time_stamp(p->base), 1, "has put down fork");
...
void	*philo2(void *arg)
{
	...
	while (1)
	{
		...
		print_safe(p, time_stamp(p->base), 2, "has taken a fork");
		start_eat = time_stamp(p->base);
		print_safe(p, start_eat, 2, "is eating");
		if (read_stop(p))
			break ;
		while (time_stamp(p->base) < start_eat + 1000)
		{
			if (read_stop(p))
				break ;
			usleep(500);
		}
		print_safe(p, time_stamp(p->base), 2, "has put down fork");
		start_sleep = time_stamp(p->base);
